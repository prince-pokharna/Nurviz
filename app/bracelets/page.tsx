...
export default function BraceletsPage() {
  ...
  // Get bracelets from centralized inventory
  const bracelets = getProductsByCategory("Bracelets")

  const materials = ["Gold", "Silver", "Platinum", "Kundan", "Beads"]
  const styles = ["Kada", "Bangle", "Chain", "Charm", "Tennis", "Traditional"]
  const occasions = ["Wedding", "Festival", "Party", "Daily Wear", "Formal"]

  const filteredBracelets = bracelets.filter((bracelet) => {
    if (bracelet.price < priceRange[0] || bracelet.price > priceRange[1]) return false
  ...
  const sortedBracelets = [...filteredBracelets].sort((a, b) => {
    switch (sortBy) {
      case "price-low":
      ...
      <main className="pt-20">
        ...
        <section className="bg-gradient-to-r from-amber-50 to-yellow-50 py-16">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h1 className="text-4xl lg:text-6xl font-bold gradient-text font-playfair mb-4">Bracelets Collection</h1>
            <p className="text-xl text-gray-600 max-w-2xl mx-auto">
              Explore our elegant collection of bracelets, from traditional kadas to contemporary designs.
            </p>
          </div>
        </section>
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                <div className="flex items-center space-x-4">
                  <h2 className="text-2xl font-semibold">Bracelets</h2>
                  <Badge variant="secondary" className="bg-amber-100 text-amber-800">
                    {sortedBracelets.length} items
                  </Badge>
                </div>
                  ...
                  <Sheet>
                    ...
                    <SheetContent side="left" className="w-80">
                      <SheetHeader>
                        ...
                        <SheetDescription>Refine your search to find the perfect bracelet.</SheetDescription>
                      </SheetHeader>
                      <div className="mt-6">
                        <FilterContent />
                      </div>
